...
export default function App() {
  ...
  const handleEmailLogin = async () => {
    ...
    setAuthUser(user);
    if(role === "master"){
      setIsMaster(true); setAuth("app"); setTab("dashboard");
    } else if(role === "admin"){
      setIsAdmin(true); setMe(null); setAuth("app"); setTab("dashboard");
      ...
      setStaffList(stData||[]);
      const myStaff = stData?.find(s=>s.auth_id === user.id);
      if(myStaff) setMe(myStaff);
  ...
  if(auth==="select") return (
    <div style={{fontFamily:"'Noto Sans JP',sans-serif",minHeight:"100vh",background:"linear-gradient(135deg,#1e3a8a,#1e293b)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{background:"white",borderRadius:24,padding:44,width:"100%",maxWidth:440,textAlign:"center",boxShadow:"0 30px 80px rgba(0,0,0,.3)"}}>
        <div style={{width:68,height:68,borderRadius:18,background:"linear-gradient(135deg,#2563eb,#7c3aed)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 16px",cursor:"pointer",userSelect:"none"}}>🏠</div>
        ...
        <div style={{marginBottom:16}}>
          ...
          <button className="btn btn-primary" style={{width:"100%",justifyContent:"center",padding:"14px",fontSize:15,background:"linear-gradient(135deg,#2563eb,#7c3aed)",color:"white",border:"none",borderRadius:8,cursor:"pointer",fontWeight:600}} onClick={handleEmailLogin}>ログイン</button>
        </div>
      </div>
    </div>
  );
