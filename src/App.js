  ...
  return(
    <div className="fade-in">
      ...
      <div className="card" style={{marginBottom:12,background:"#eff6ff",border:"1px solid #bfdbfe"}}>
        <div style={{fontSize:12,color:"#1e40af"}}>💡 staff_membersテーブルで管理。PINは個人ごとに設定してください。</div>
      </div>
      {adding&&(
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:10,marginBottom:10}}>
            {[["名前 *","name","text"],["フリガナ","kana","text"],["電話","tel","tel"],["メール","email","email"],["入職日","hire_date","date"],["PIN（4〜8桁）","pin","password"]].map(([label,k,type])=>(
              <div key={k}><label style={{fontSize:12,color:"#64748b",display:"block",marginBottom:3}}>{label}</label>
                <input className="input" type={type} autoComplete="new-password" value={form[k]||""} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))}/></div>
            ))}
          </div>
